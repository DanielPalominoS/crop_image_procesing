...
public:
    ...
    : it_(nh_)
  {
    ...
    saveService = nh_.advertiseService("/test_server/save",&ImageConverter::saveIm,this);
    cropService = nh_.advertiseService("/test_server/findCrop",&ImageConverter::findCropFeatures,this);
    /*cv::namedWindow(LINES_WINDOW);
    cv::namedWindow(CONTOUR_WINDOW);
    cv::namedWindow(ROWS_WINDOW);*/
  }
    ...
    : it_(nh_)
  {
    ...
    image_sub_ = it_.subscribe("/down_cam1/camera/image", 1,
      &ImageConverter::imageCb, this);
    //image_pub_ = it_.advertise("/image_converter/output_video", 1);
    saveService = nh_.advertiseService(name+"/save",&ImageConverter::saveIm,this);
    cropService = nh_.advertiseService(name+"/findCrop",&ImageConverter::findCropFeatures,this);
    /*cv::namedWindow(LINES_WINDOW);
    cv::namedWindow(CONTOUR_WINDOW);
    cv::namedWindow(ROWS_WINDOW);*/
  }
  ...
  void hsv_mask(cv::Mat *src,cv::Mat *dst ,int low_H,int low_S,int low_V,int high_H,int high_S,int high_V){
      cv::Mat hsv;
  ...
  void  find_contour(cv::Mat *src,std::vector<cv::Point>* contour,cv::RotatedRect* minRect,double gsd){
    int morph_size;
    int operation;
    cv::Mat element;
    cv::Mat img;
...
{
  std::setprecision (10);
  std::string name("test_server");
  ros::init(argc, argv, "test_server");
  ImageConverter ic;
  ImageConverter ic_("test_ser");
